...
fn detect_names(text: &str, existing: &[PiiSpan]) -> Vec<PiiSpan> {
    ...
    for cap in verb_context_regex().captures_iter(text) {
        if let Some(m) = cap.get(1) {
            let first_lc = m
                ...
                .unwrap_or("")
                .to_lowercase();
            if first_names.contains(first_lc.as_str()) || last_names.contains(first_lc.as_str()) {
                push!(m.start(), m.end(), m.as_str());
            }
        }
    }
    ...
    for m in cap_seq_regex().find_iter(text) {
        ...
        let is_name =
            if first_names.contains(first_lc.as_str()) && last_names.contains(last_lc.as_str()) {
...
mod tests {
    ...
    fn detects_single_surname_after_mr() {
        ...
        assert!(
            spans
                .iter()
                .any(|s| s.entity_type == EntityType::Name && s.text == "Smith"),
